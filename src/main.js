...
function setByPath(path, value, {renderNow=true}={}) {
  ...
  if (path === "race") character.raceVariant = "";
  ...
  app.innerHTML = `
    ...
    <section class="sheet">
      ...
      <section class="panel" data-panel-id="abilities">
        <h2>Abilities</h2>
        <div class="ability-grid">${abilityOrder.map(a => { const d=abilityState.details[a]; const mods=[...d.adds.map(x=>`${x.label} ${x.value>=0?"+":""}${x.value}`),...d.overrides.map(x=>`${x.label} sets at least ${x.value}`),...d.maxima.map(x=>`${x.label} ${x.increase?`raises max +${x.increase}`:`max ${x.set}`}`)]; return `<div class="ability" title="${esc(mods.length?`Base ${d.base}; ${mods.join("; ")}`:`Base ${d.base}; no active modifiers`)}"><strong>${abilityNames[a]}</strong><label class="ability-base"><span>Base</span><input data-path="baseAbilities.${a}" type="number" min="1" max="30" value="${character.baseAbilities[a]}"></label><div class="ability-final"><span>Final</span><b>${character.abilities[a]}</b></div><div class="mod">${formatMod(abilityMod(character.abilities[a]))}</div></div>`; }).join("")}</div><p class="muted">Edit Base scores. Race, ASIs, half-feats, and active magic-item effects recalculate the Final score automatically; hover an ability for its modifier sources.</p>
...
function characterChoicesPanel(){
  ...
  const racialHtml=racial.data ? `<section class="choice-group"><h3>Racial Ability Scores</h3><p class="muted">${esc(racial.rule.text||fixedAbility||"No ability score choice")}</p>${fixedAbility?`<div class="choice-summary"><strong>Fixed:</strong> ${esc(fixedAbility)}</div>`:""}${racial.groups.map(({group,selected,offset})=>`<div class="choice-grid">${Array.from({length:group.count},(_,i)=>`<label class="mini-field"><span>+${group.amount} choice ${i+1}</span>${choiceSelect(`racial:${offset+i}`,selected[i],group.options.map(k=>({id:k,label:ABILITY_LABELS[k]})),`data-racial-ability-slot="${offset+i}"`,`Choose ability...`,selected)}</label>`).join("")}</div>`).join("")}${racial.missing?`<div class="choice-needed">${racial.missing} racial ability choice${racial.missing===1?"":"s"} remaining</div>`:`<div class="good">Racial ability choices complete</div>`}<p class="muted">These bonuses now apply automatically to the Final ability scores shown in the Abilities panel.</p></section>` : "";
...
function drawCharacterBuilder(){
  ...
  creationDialog.querySelectorAll('[data-builder-field]').forEach(el=>el.addEventListener('change',()=>{ const key=el.dataset.builderField; creationDraft[key]=el.value; if(key==='race')creationDraft.raceVariant=''; if(key==='background')creationDraft.backgroundVariant=''; drawCharacterBuilder(); }));
